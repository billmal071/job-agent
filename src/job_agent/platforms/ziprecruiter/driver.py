"""ZipRecruiter platform driver implementation."""
...
from job_agent.platforms.ziprecruiter.discovery import ZipRecruiterDiscovery
...
class ZipRecruiterDriver(PlatformDriver):
    """ZipRecruiter job platform driver."""
    ...
    def __init__(self, settings: Settings, browser_manager: BrowserManager):
        ...
        platform_cfg = settings.platforms.ziprecruiter
        ...
        self._discovery: ZipRecruiterDiscovery | None = None
        self._applicator: ZipRecruiterApplicator | None = None

    def login(self, username: str, password: str) -> None:
        context = self.browser.get_context("ziprecruiter")
        auth = AuthManager(context)
        self._page = auth.login(Platform.ZIPRECRUITER, username, password)
        self._discovery = ZipRecruiterDiscovery(self._page, self.rate_limiter)
        self._applicator = ZipRecruiterApplicator(
            self._page, self.rate_limiter, self.settings
        )
        self.browser.save_state("ziprecruiter")
        log.info("ziprecruiter_driver_ready")
    ...
    def close(self) -> None:
        ...
        if self._page and not self._page.is_closed():
            self._page.close()
        self._page = None
        log.info("ziprecruiter_driver_closed")
